...
# --- Helper Function to Combine JSON Files ---
def combine_faq_files(source_dir: Path) -> List[Dict[str, Any]]:
    """
    Finds all .json files in the source directory, reads them,
    and combines their contents (assuming each file contains a list of QA dicts).
    """
    combined_data = []
    ...
    logger.info(f"Found {len(json_files)} JSON files to combine in {source_dir}...")
    for json_file in json_files:
        try:
            with open(json_file, "r", encoding="utf-8") as f:
                data = json.load(f)
                if isinstance(data, list):
                    # Filter out any potential empty items if necessary
                    valid_items = [item for item in data if isinstance(item, dict) and item.get("question") and item.get("answer")]
                    combined_data.extend(valid_items)
                    logger.debug(f"Added {len(valid_items)} items from {json_file.name}")
                else:
    ...
    logger.info(f"Total combined FAQ pairs: {len(combined_data)}")
...
def main(local_source_dir: str = str(LOCAL_EXTRACTED_FAQ_DIR)):
    """
    Local entrypoint:
    1. Combines local JSON files.
    2. Calls the Modal function to upload the combined data.
    """
    logger.info(f"Starting FAQ data combination and upload process.")
    source_path = Path(local_source_dir)

    # 1. Combine local files
    logger.info(f"Combining JSON files from local directory: {source_path}")
    combined_faq_data = combine_faq_files(source_path)

    if not combined_faq_data:
    ...
    try:
        upload_combined_data.remote(combined_faq_data, COMBINED_FILENAME)
